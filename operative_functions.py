...
def order_file(agent_full_name, order_code, steel_grade, thickness, \
               ...
               oel_sorte, single_reduction, assivieru_ngkz, each_coil_price, list_ware, string_operations, wait_time,\
               shdate):
...
def set_agent_parameters(agent_name, agent_full_name,ancho,espesor,\
                         largo,param_f,artikel_gruppe,oel_sorte,single_reduction,assivieru_ngkz, sgrade,location,ordr):
...
def production_cost(coil_df, i):
    z = coil_df.loc[i,'HRC Width (mm)'] - coil_df.loc[i,'FinalWidth']
    n = coil_df.loc[i,'HRC Thickness (mm)'] - coil_df.loc[i,'Final Thickness (mm)']
    cost = float(z * 4 + n * 2)
    return cost
...
def energy_cost(df_parameters_energy, price_energy_consumption,coil_msgs_df,i):
    ...
    if to == 'va09' or to == 'va10' or to == 'va11' or to == 'va12':
        ...
        power = a + coil_msgs_df.loc[i,'FinalWidth'] * b + coil_msgs_df.loc[i,'Final Thickness (mm)'] * c + speed * f + tinlayerup * d + tinlayerdown * e
...
def va_rules(coil_msgs_df,i, winner_df):
    ...
    if to == 'va08':
        ...
        if abs((coil_msgs_df.loc[i,'HRC Width (mm)'] - coil_msgs_df.loc[0,'FinalWidth'])) > 120:                                               
            accept = accept*0.2
        ...
        if len(winner_df)>0:
            if abs((winner_df['HRC Thickness (mm)']-coil_msgs_df.loc[i,'HRC Thickness (mm)'])) > 0.1:
                accept = accept*0.3
...
def va_bid_evaluation(df_parameters_energy,coil_msgs_df, price_energy_consumption, winner_df):
    ...
    for i in range(coil_msgs_df.shape[0]):
        ...
        n = coil_msgs_df.loc[i, 'transport_cost']
        energy = coil_msgs_df.loc[i, 'energy_cost']
        coil_msgs_df.loc[i, 'cost'] = m + n + energy     ese cost en los corchetes era minimum price
    for i in range(coil_msgs_df.shape[0]):
    ...
    if step == 'bid':
        ...
        results = coil_msgs_df[['agent_type', 'id', 'coil_jid', 'bid', \
                            ...
                            'espesor', 'ship_date','budget_remaining',\
                            'counterbid','profit','User_name_va','plant_rule']]
...
def va_result(coil_ofertas_df, jid_list):
    df = pd.DataFrame([], columns=['Coil', 'cost'])
    '''
    if step == 'counterbid' :
    ...
    for iz in range(len(lc)):
        ...
        if df.shape[0] == 0:
            ...
            lres.append({'id':id,'code':z,'loc':pz,'bdg':bdg,'orden':odn,\
                         ...
                         'assivieru_ngkz':str(assivieru_ngkz), 'sdate':sdt,'st':'ini'})
        else:
            id = df.iloc[0]['id']
            lres.append({'id':id,'code':z,'loc':pz,'bdg':bdg,'orden':odn,\
                         'ph':pth,'ancho':str(int(ach)),'sg':sgd,\
                         ...
                         'parF':str(parF), 'artikel_gruppe':str(artikel_gruppe),\
                         'oel_sorte':str(oel_sorte), 'single_reduction': str(single_reduction),\
                         'assivieru_ngkz':str(assivieru_ngkz),\
...
def set_agent_parameters_coil(my_dir, my_name, my_full_name, \
                              ...
                              single_reduction, assivieru_ngkz, path, sdate):
